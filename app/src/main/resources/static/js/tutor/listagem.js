document.addEventListener("DOMContentLoaded", () => {
    listagemTutores.functions.pagination.carregarListagemTutoresPaginada();
});

var listagemTutores = {
    elements: {
        alert: () => {
            return document.querySelector("#listagem-tutores-container div[role=alert]");
        },
        table: () => {
            return document.querySelector("#listagem-tutores-container table");
        }
    },
    functions: {
        alert: {
            show: (message, type) => {
                let alert = listagemTutores.elements.alert();
        ...
        table: {
            show: () => {
                let table = listagemTutores.elements.table();
                table.hidden = false;
            },
            hide: () => {
                let table = listagemTutores.elements.table();
            ...
            construirLinha: (tutor) => {
                ...
                const urlTutor = `${baseUrl}/tutores/${tutor.id}`;
                ...
                return `
                    <tr id="cao-item-list">
                        ...
                        <td class=" text-center">
                            ...
                            <a class="icon-link icon-link-hover ms-3" href="${urlTutor}/editar" style="--bs-icon-link-transform: translate3d(0, -.125rem, 0);">
        ...
        pagination: {
            carregarListagemTutoresPaginada: () => {
                ...
                $('#listagem-tutores-container tfoot')
                    .pagination({
                        dataSource: `${baseUrl}/rest/tutores`,
                        pageSize: 10,
                        locator: "content",
                        ajax: {
                        ...
                        formatAjaxError: (jqXHR, textStatus, errorThrown) => listagemTutores.functions.pagination.onError(jqXHR, textStatus, errorThrown),
                        callback: (data, pagination) => listagemTutores.functions.pagination.onSuccess(data, pagination)
                    });
            },
